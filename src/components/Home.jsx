...
export default function Home() {
  ...
  return (
    <div>
     ...
     {paginationArray.length > 1 && !searchResult.length ? (<div className=' py-5 flex justify-center items-center space-x-3 '>
     {paginationArray.map((_, index) => {
      return <button key={index}
      onClick={()=> fetchMorePosts(index)}
       className={index === pageNo ? 'text-blue-500 border-b-2 border-b-blue-500':'text-gray-500'}>
         {index + 1}</button>
    })}
     </div>):null}
    </div>
